...
mod tests {
    ...
    fn parses_direct_testsuite_root() {
        ...
        assert_eq!(cases[0].duration_ms, Some(250));
        assert_eq!(cases[0].message.as_deref(), Some("bad"));
    }

    #[test]
    fn parses_testsuites_wrapper() {
        let cases = parse_test_xml(
            r#"<testsuites><testsuite><testcase name="ok"/><testcase name="skip"><skipped message="later"/></testcase></testsuite></testsuites>"#,
        )
        .unwrap();
        assert_eq!(cases.len(), 2);
        assert_eq!(cases[0].status, TestStatus::Passed);
        assert_eq!(cases[1].status, TestStatus::Skipped);
        assert_eq!(cases[1].message.as_deref(), Some("later"));
    }
}
