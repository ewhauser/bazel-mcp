...
transports=${BEP_TRANSPORTS:-"tail bes"}
...
printf 'version\ttransport\tcase\tstate\texit\n'
...
EOF
  ...
  for transport in $transports; do
    case_output="$matrix_root/$version/$transport/cases.tsv"
    mkdir -p "$(dirname "$case_output")"
    python3 scripts/test-mcp-smoke.py \
      --workspace "$workspace" --server "$server" --bazel "$bazel" \
      --root "$matrix_root/$version/$transport/runtime" --wrapper \
      --bep-transport "$transport" >"$case_output"
    while IFS= read -r row; do
      printf '%s\t%s\t%s\n' "$version" "$transport" "$row"
    done <"$case_output"
  done
done
